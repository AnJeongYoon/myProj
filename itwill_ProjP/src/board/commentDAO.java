...
public class commentDAO {
	...
	public List<commentBean> getCommentList(int startRow, int pageSize, int boardNum){
		...
		try {
			...
			sql = "SELECT * FROM p_comment WHERE boardNum = ? ORDER BY comm_group ASC, comm_seq ASC LIMIT ?, ?";
	...
	public commentBean getComment(int boardNum){
		...
		try {
			...
			con = getConnection();
			//3. sql작성
			sql = "SELECT * FROM p_comment WHERE boardNum = ? ORDER BY comm_group ASC";
...
//			pstmt.executeUpdate();
			...
			pstmt.setString(5, comBean.getIp());
			pstmt.setString(6, comBean.getComment());
			pstmt.setInt(7, comBean.getComm_group());	//no 주글번호 기준 == comm_group그룹번호
			pstmt.setInt(8, comBean.getComm_seq()+1);
			pstmt.setInt(9, comBean.getComm_lev()+1);
