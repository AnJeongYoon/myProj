...
public class Board2DAO {
	...
	public int insertBoard(BoardBean bBean){
		...
		try {
			...
			sql = "SELECT max(no) FROM p_board2";	//가장 큰 글번호 가져오기
			...
			//insert
			sql = "INSERT INTO p_board2(no, user_id, user_name, ip, subject, content, file, reg_date, readCnt, repCnt, commCnt, re_group, re_seq, re_lev)"
					+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, 0, 0)";
	...
	public int getBoardCount(){
		...
		try {
			...
			con = getConnection();
			//3 sql 전체 글갯수 가져오기
			sql = "SELECT count(*) FROM p_board2";
			pstmt = con.prepareStatement(sql);
			//4. rs=select한 결과 값
			rs = pstmt.executeQuery();
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize){
		...
		try {
			...
			sql = "SELECT * FROM p_board2 ORDER BY re_group DESC, re_seq ASC LIMIT ?, ?";
		//	sql = "SELECT * FROM p_board2 ORDER BY no DESC LIMIT ?, ?";
	...
	public void updateReadCount(int no){
		...
		try {
			...
			con = getConnection();
			//3. sql작성
			sql = "UPDATE p_board2 SET readCnt = readCnt +1 WHERE no = ?";
	...
	public BoardBean getBoard(int no){
		...
		try {
			...
			con = getConnection();
			//3. sql작성
			sql = "SELECT * FROM p_board2 WHERE no = ?";
	...
	public int delBoard(int no, String user_id){
		...
		try {
			...
			sql = "SELECT user_id FROM p_board2 WHERE no = ?";
			...
			if(rs.next()){
				if (user_id.equals(rs.getString("user_id"))) {
					check = 1;
					//sql = num에 해당하는 글 삭제
					sql = "DELETE FROM p_board2 WHERE no =?";
					pstmt = con.prepareStatement(sql);
					pstmt.setInt(1, no);
					pstmt.executeUpdate();
	...
	public int updateBoard(BoardBean bBean, String user_id){
		...
		try {
			...
			if(rs.next()){
				if (user_id.equals(rs.getString("user_id"))) {
					sql = "UPDATE p_board2 SET subject=?, content=?, file=? WHERE no = ?";
	...
	public int reInsertBoard(BoardBean bBean){
		...
		try {
			...
			sql = "SELECT max(no) FROM p_board2";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {	//글번호가 있으면
				//답변글번호 = 글번호 + 1
...
//			sql = "UPDATE p_board2 SET re_seq = re_seq+1, repCnt = repCnt+1 WHERE re_group=? AND re_seq > ?";
//			
//			pstmt = con.prepareStatement(sql);
...
//			pstmt.executeUpdate();
	...
	public int getCount(String search){
		...
		try {
			con = getConnection();
			sql = "SELECT count(*) FROM p_board2 WHERE subject like ?";
			pstmt = con.prepareStatement(sql);
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize, String search){
		...
		try {
			...
			sql = "SELECT * FROM p_board2 WHERE subject like ? ORDER BY re_ref DESC, re_seq ASC LIMIT ?, ?";
			pstmt = con.prepareStatement(sql);
